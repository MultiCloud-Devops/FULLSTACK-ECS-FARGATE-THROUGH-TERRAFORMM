...
resource "aws_ecs_task_definition" "back-task" {
  ...
  container_definitions = jsonencode([
    {
      name      = "backend"
      image     = "442426876097.dkr.ecr.us-east-1.amazonaws.com/backend:latest"
      cpu       = 256
      memory    = 512
      essential = true
      ...
      environment = [
        { name = "DB_HOST", value = "book-rds.cehcum82mrbk.us-east-1.rds.amazonaws.com" },
        { name = "PORT", value = "3306" },
        { name = "DB_USERNAME", value = "admin" },
        { name = "DB_PASSWORD", value = "AdminPassword123" }
      ]
    }
  ])
}
